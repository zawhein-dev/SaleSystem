...
$numberTitle = ($page * $limit) - $limit;
    if (isset($_GET['search_data'])) {
            ...
            $row =  get_branch_with_search_data($mysqli, $searchData);
            $row_count = COUNT($row->fetch_all());
            $pagination_link = ceil($row_count / 3); 
            $branches = get_branch_with_offset($mysqli, $offset, $limit,$searchData);
    }else{
        $row = get_branch($mysqli);
        $row_count  = COUNT($row->fetch_all()); //get number of users
        $pagination_link = ceil($row_count / 3);
        $branches = get_search_branch_with_offset($mysqli, $offset, $limit);
    }
?>
