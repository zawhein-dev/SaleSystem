...
<div class="main  w-100 bg-white">
    <div class="main-content w-50 mx-auto">
        ...
        <form method="POST" class="form-group">
            ...
            <div style="height: 13px; line-height: 13px; font-size: 14px; margin-top: 3px;">
                <i class="text-danger text-sm-start"><?= $productErr ?></i>
            </div>
            ...
            <select name="branch" id="branch" class="form-select form-select-sm">
                ...
                while ($branch_data = $branches->fetch_assoc()) { ?>
                    <option value="<?= $branch_data['branch_id'] ?>" <?php if ($branch == $branch_data['branch_id'])
                          echo "selected"; ?>><?= $branch_data['branch_name'] ?></option>
                <?php } ?>
            </select>
            <div style="height: 13px; line-height: 13px; font-size: 14px; margin-top: 3px;">
                <i class="text-danger text-sm-start"><?= $branchErr ?></i>
            </div>
            ...
            <div class="form-input-group-sm text-center">
                ...
                <?php if (isset($_GET['duplicate']) && isset($_GET['branch_product_id'])) {
                    ...
                    <a href="./add_branch_product.php?branch_product_id=<?= $returnToUpdateWithBranchProductId ?>"
                        class="btn btn-success">Updete here</a>
                <?php } ?>
            </div>
        </form>
    </div>
