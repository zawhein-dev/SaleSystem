...
if (isset($_SESSION["item_list"])) {
    ...
    if (count($item_array) > 0) {
        ...
        foreach ($item_array as $index => $item) {
            ...
            echo '<div class="col-2 text-end">' .number_format($subtotal, 2, ".", ",") . '</div>';
            // echo '</div>';
            echo '</div>'; 
            }
        
        } 
        echo '<div class="row mt-3 mb-2"><div class="col text-end"><strong>Net Total: ' . number_format($net_total, 2, ".", ",") . '</strong></div></div>';
